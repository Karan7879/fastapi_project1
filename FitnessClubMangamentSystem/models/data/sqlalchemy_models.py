...
class Login(Base):
    ...
    trainers = relationship('Profile_Trainer',back_populates="login",uselist=False)
...
class Profile_Members(Base):
    ...
    id  = Column(Integer,ForeignKey('login.id'),primary_key=True,index=True)
    firstname = Column(String,index=False,unique=False)
    lastname = Column(String,unique=False,index=False)
    age = Column(Integer,unique=False,index=False)
    height = Column(Integer,unique=False,index=False)
    weight =  Column(Integer,unique=False,index=False)
...
class Gym_Class(Base):
    ...
    trainer_id = Column(Integer,ForeignKey("profile_trainer.id"),unique=False,index=False)
